/*
 *  HomeAssistant-MQTT 1.5 (WT32-ETH01 Ethernet)
 *
 ...
 *
 *  Release notes:
 *    1.5 - Added DSC Classic series support, Modified for WT32-ETH01 Ethernet
 ...
 *
 *                                         +--- dscClockPin  // Default: 5
 *      DSC Yellow --- 33k ohm resistor ---|
 *                                         +--- 10k ohm resistor --- Ground
 *
 *                                         +--- dscReadPin   // Default: 35
 ...
 *                 |
 *                 |                       +--- dscPC16Pin   // Default: 32
 *                 +-- 33k ohm resistor ---|
 *                                         +--- 10k ohm resistor --- Ground
 *
 *      Virtual keypad (optional):
 *      DSC Green ---- NPN collector --\
 *                                      |-- NPN base --- 1k ohm resistor --- dscWritePin  // Default: 33
 *            Ground --- NPN emitter --/
 *
...
// Ethernet settings (optional - will use DHCP if not specified)
IPAddress local_IP;       // Leave empty for DHCP, or set like: IPAddress local_IP(192, 168, 1, 100);
IPAddress gateway;        // Leave empty for DHCP, or set like: IPAddress gateway(192, 168, 1, 1);
IPAddress subnet;         // Leave empty for DHCP, or set like: IPAddress subnet(255, 255, 255, 0);
IPAddress primaryDNS;     // Leave empty for DHCP, or set like: IPAddress primaryDNS(8, 8, 8, 8);
IPAddress secondaryDNS;   // Leave empty for DHCP, or set like: IPAddress secondaryDNS(8, 8, 4, 4);
...
// Configures the Keybus interface with the specified pins for WT32-ETH01 - dscWritePin is optional, leaving it out disables the
// virtual keypad. Using different pins than the original to avoid conflicts with Ethernet
#define dscClockPin 5   // Available GPIO pins on WT32-ETH01
#define dscReadPin  35  // Available GPIO pins on WT32-ETH01  
#define dscPC16Pin  32  // DSC Classic Series only
#define dscWritePin 33  // Optional for virtual keypad
...
void onEvent(arduino_event_id_t event) {
  switch (event) {
    case ARDUINO_EVENT_ETH_START:
      ...
      ETH.setHostname("dscKeybusInterface");
      break;
...
void setup() {
  ...
  // Set static IP if configured (optional)
